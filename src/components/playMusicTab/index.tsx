...
const PlayMusicTab = ({ song }: PlayMusicType) => {
  ...
  const nextSong = () => {
    const songSelect = songList?.find(
      (songItem) => songItem.name === song.name
    );
  ...
  const randomSong = () => {
    ...
    if (songList && songSelect) {
      ...
      if (index === randomIndex) {
        if (index === 0)
          return songDispatch(setSongInfo(songList[randomIndex + 2]));
  ...
  return (
    <IonCol
      className="play-music-tab"
      style={{ display: song.name ? "block" : "none" }}
    >
      <IonFabButton
        color="success"
        className="toogle-zoom-tab"
        onClick={() => setZoomTab(!zoomTab)}
      >
      ...
      <audio
        ...
        onEnded={
          isRepeatSong ? repeatSong : isRandomSong ? randomSong : nextSong
        }
      />
      {zoomTab ? (
        <div className="song-tab-zoom-out">
          ...
          <IonRow className="ion-justify-content-between">
            ...
            <IonText color="success">
              {timeOfAudio ? timeOfAudio : "00:00"}/{durationFormart}
            </IonText>
          </IonRow>
          <IonRange
          ...
          <IonRow className="ion-justify-content-between">
            <IonButtons>
              <IonButton
                color={isRandomSong ? "success" : "dark"}
                onClick={() => setIsRandomSong(!isRandomSong)}
              >
            ...
            <IonButtons>
              ...
              <IonButton
                color="success"
                onClick={isRandomSong ? randomSong : nextSong}
              >
            ...
            <IonButtons>
              <IonButton
                color={isRepeatSong ? "success" : "dark"}
                onClick={() => setIsRepeatSong(!isRepeatSong)}
              >
                <IonIcon icon={repeat} />
              </IonButton>
            </IonButtons>
